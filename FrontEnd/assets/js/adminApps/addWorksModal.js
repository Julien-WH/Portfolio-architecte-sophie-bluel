...
async function labelCategories() {
    ...
    inputCategories.innerHTML = '<option hidden disabled selected value> Sélectionnez une catégorie </option>'; // Ajout d'une option vide
...
function showFile() {
  ...
  function handleFileErrors(file, validExtensions, maxSize) {
    ...
    if (!validExtensions.includes(file.type)) {
      appendErrorMessage("Format de fichier invalide. Les formats valides sont: .jpeg, .jpg, .png");
    }
  ...
  function validateForm() {
    const isFormValid = titleInput.value.trim() !== "" &&
